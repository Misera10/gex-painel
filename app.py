...
st.markdown("""
    <style>
    /* Dark Theme Premium com Gradiente */
    .stApp { 
    ...
    code { 
        ...
        text-align: center; 
        border-radius: 8px; 
        background: linear-gradient(135deg, #161a25, #1f2533) !important; 
    ...
    /* Header Box */
    .header-box {
        background: linear-gradient(135deg, rgba(30, 34, 45, 0.9), rgba(26, 31, 46, 0.9));
        padding: 25px;
        border-radius: 12px;
        border: 1px solid rgba(0, 255, 170, 0.2);
        margin-bottom: 20px;
    ...
    [data-testid="stSidebar"] {
        background: linear-gradient(180deg, #1a1f2e 0%, #0b0e14 100%);
...
def generate_trade_signal(spot, basis, levels, regime, vix_data):
    """Gera sinal de trade estruturado ancorado no preço do ES (Futuros)"""
    signal = {
        "direction": None,
        "confidence": 0,
        "entry_zone": None,
    ...
    # Confirmação 2: Distância para Walls (Cálculo de Espaço Físico no ES)
    alvo_pontos = es_spot * 0.005 # ~0.5%
    ...
    if signal['direction'] == "SHORT 📉":
        ...
        signal['invalidation'] = es_zg + 10 # Stop técnico 10 pts acima do Zero Gama ES
        ...
        signal['invalidation'] = es_zg - 10 # Stop técnico 10 pts abaixo do Zero Gama ES
        signal['entry_zone'] = f"{es_spot:.2f} - {es_spot+5:.2f}"
...
def generate_trade_report(signal, levels, spot, basis, timestamp):
    """Gera relatório em Markdown ancorado no ES"""
...
---
*Este relatório valida o espaço matemático com base no fluxo institucional de opções na CBOE.*
*Execute disciplinadamente.*
    """
    return report
...
def render_setup_score(score, max_score=3, regime="NEGATIVO"):
    ...
    <div class="metric-card animate-slide-in">
        <div style="display:flex; justify-content:space-between; align-items:center; margin-bottom:12px;">
            ...
            <span class="badge badge-{'positive' if score>=2 else 'warning' if score==1 else 'negative'}">
                {regime}
            </span>
        </div>
        <div style="display:flex; align-items:baseline; gap:10px;">
            <div style="font-size:42px; font-weight:900; color:{color}; line-height:1;">
                {score}/{max_score}
            </div>
        ...
        <div style="margin-top:10px; font-size:12px; color:#8A94A6;">
            Validação matemática de deslocamento para alvo de 0.5% (ES).
        </div>
    </div>
    """, unsafe_allow_html=True)
...
def render_gamma_profile(df_chart, spot_price, zero_gamma, call_wall, put_wall):
    ...
    rules = alt.Chart(ref_data).mark_rule(strokeDash=[4, 4], strokeWidth=2).encode(
        y='level:Q', color=alt.Color('color:N', scale=None),
...
with st.sidebar:
    ...
    st.markdown("---")
    st.caption("🔐 GEX ULTRA ELITE v3.0\n\n*Validação Quantitativa Direta*")
