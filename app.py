...
def export_to_tradingview(levels, basis, es_spot, timestamp):
    ...
    def add_shape(price, text, color, style, width):
        if not pd.isna(price) and price > 0:
            es_price = price + basis
            tv_data["shapes"].append({
                "shape": "horizontal_line",
                "text": text.format(es_price),
                "price": es_price,
                "color": color,
                "linewidth": width,
                "linestyle": style
    ...
    add_shape(levels.get('c_wall'), "🔴 Call Wall {:.2f}", "#FF4444", 2, 2)
    add_shape(levels.get('z_gama'), "🔄 Zero Gamma {:.2f}", "#00FFAA", 0, 3)
    add_shape(levels.get('p_wall'), "🟢 Put Wall {:.2f}", "#00CC88", 2, 2)
    add_shape(levels.get('vt'), "📊 Vol Trigger {:.2f}", "#FFCC00", 1, 1)
    add_shape(levels.get('c_wall_0dte'), "🔴 CW 0DTE {:.2f}", "#FF8888", 1, 2)
    add_shape(levels.get('p_wall_0dte'), "🟢 PW 0DTE {:.2f}", "#88FF88", 1, 2)
    add_shape(levels.get('l1'), "📈 L1 {:.2f}", "#4ECDC4", 2, 1)
    add_shape(levels.get('c1'), "📉 C1 {:.2f}", "#95E1D3", 2, 1)
    add_shape(levels.get('c4'), "💀 C4 {:.2f}", "#F38181", 2, 1)

    tv_data["marks"] = [{
        "time": timestamp.strftime("%Y-%m-%d"),
        "price": es_spot,
        "text": f"💰 ES Spot={es_spot:.2f}",
        "label": "price",
        "color": "#FFFFFF"
    }]
    return tv_data
    ...
    with st.spinner("⚡ Calculando derivativos, sincronizando Basis ES e avaliando setup..."):
        try:
            ...
            with col_signal:
                ...
                with col_btn1:
                    if signal['direction']:
                        st.download_button(label="📄 BAIXAR RELATÓRIO", data=generate_trade_report(signal, levels_dict, spotPrice, basis, datetime.now()), file_name=f"GEX_Execucao_{datetime.now().strftime('%Y%m%d_%H%M')}.md", mime="text/markdown", use_container_width=True)
                with col_btn2:
                    tv_json = export_to_tradingview(levels_dict, basis, es_spot, datetime.now())
                    st.download_button(label="📥 EXPORTAR JSON (TRADINGVIEW)", data=json.dumps(tv_json, indent=2), file_name=f"GEX_TV_Levels_{datetime.now().strftime('%Y%m%d')}.json", mime="application/json", use_container_width=True)
